...
class AllToAllStatus:
    ...
    def init(group: dist.ProcessGroup, num_split: int, split_dim: int) -> None:
        ...
        if not AllToAllStatus.initialized:
            ...
            dist.broadcast(nccl_unique_id, dist.distributed_c10d._get_global_rank(group, 0), group)
    ...
    def backward(ctx: Any, grad_output):
        if FAKE_A2A_SCALE > 1:
            for i in range(FAKE_A2A_SCALE - 1):
                simple_all_to_all(grad_output, group=ctx.group)
    ...
    def transform(input, input_dim, output_dim, group=None, background=False, use_2dh=False):
        ...
        if background:
            ...
            if input_dim == 0:
                ...
                if FAKE_A2A_SCALE > 1:
                    for i in range(FAKE_A2A_SCALE - 1):
                        simple_all_to_all(reshaped_input, group)
                output, f_wait = simple_all_to_all(reshaped_input, group, background=True)

                def f_async():
                    f_wait()
                    local_input = RestoreBackward.apply(output, reshaped_input, group)
