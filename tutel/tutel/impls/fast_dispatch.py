...
def extract_critical(scores, top_k, loss_fn=losses.gshard_loss, capacity_factor=1.0, batch_prioritized_routing=False, normalize_gate=True, alignment=1, group=None, inequivalent_tokens=False):
    ...
    return (num_global_experts, indices_s, locations_s, gates_s, capacity), topk_indices
...
def extract_critical_encode(ctx, x, scores, top_k, loss_fn=losses.gshard_loss, capacity_factor=1.0, batch_prioritized_routing=False, normalize_gate=True, alignment=1, group=None, inequivalent_tokens=False):
    ...
    tokens_per_expert = megablocks_ops.histogram(top_experts.view(-1), num_global_experts)
...
#topk=1
def fast_extract_critical(scores, capacity_factor=1.0):
    num_global_experts = int(scores.size(1))
    topk_indices = torch.topk(scores, top_k, dim=1).indices
    ...
    samples_per_expert = (num_samples + num_global_experts - 1) // num_global_experts
    if capacity_factor > 0:
        capacity = int(capacity_factor * samples_per_expert)
