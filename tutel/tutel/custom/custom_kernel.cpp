...
static void invoke(const std::vector<torch::Tensor> &ts, const std::vector<long> &args, const std::vector<int> &blocks, int fd) {
  ...
  if (blocks.size() == 0)
    jit_execute(ppargs, fd, dev, _gms[fd].blocks, _gms[fd].threads, at::cuda::getCurrentCUDAStream().stream());
  else if (blocks.size() == 1)
    jit_execute(ppargs, fd, dev, dim3(blocks[0]), _gms[fd].threads, at::cuda::getCurrentCUDAStream().stream());
  else if (blocks.size() == 2)
    jit_execute(ppargs, fd, dev, dim3(blocks[0], blocks[1]), _gms[fd].threads, at::cuda::getCurrentCUDAStream().stream());
  else
    jit_execute(ppargs, fd, dev, dim3(blocks[0], blocks[1], blocks[2]), _gms[fd].threads, at::cuda::getCurrentCUDAStream().stream());
}
...
inline at::cuda::CUDAStream& get_comp_stream() {
  static at::cuda::CUDAStream comp_stream = at::cuda::getStreamFromPool();
  return comp_stream;
}
...
static std::vector<torch::Tensor> nccl_all_to_all_scatter_async(
  ...
  for (int i = 0; i < num_split; i++) {
    ...
    output_list[i] = torch::empty(output_size, options, torch::MemoryFormat::Contiguous);
  }
...
static torch::Tensor nccl_all_to_all_gather_async(
  ...
  auto options = torch::TensorOptions().dtype(input_list[0].dtype()).device(input_list[0].device());
  torch::Tensor output = torch::empty(output_size, options, torch::MemoryFormat::Contiguous);
...
int all_to_all_with_scale(torch::Tensor & input, torch::Tensor & output, int scale=1){
  CHECK_CUDA(input);
  ...
  int nranks = g_world_size;
  size_t rankdiff = input.nbytes() / nranks ;

  CHECK_EQ(0, ncclGroupStart());
  for (int s = 0;s < scale; s++){
    for (int g = 0; g < nranks; g++) {
        CHECK_EQ(0, ncclRecv(((char*)output_buff) + g * rankdiff, rankdiff, ncclInt8, g , g_nccl_comm, at::cuda::getCurrentCUDAStream().stream()));
        CHECK_EQ(0, ncclSend(((char*)input_buff) + g * rankdiff, rankdiff, ncclInt8, g , g_nccl_comm, at::cuda::getCurrentCUDAStream().stream()));
    }
  }
  CHECK_EQ(0, ncclGroupEnd());
...
std::vector<at::Tensor> pipe_forward(int64_t chunk_size, int64_t seq_groups, int64_t real_batch, int64_t chunk_len, 
  float hidden_dropout,
  at::Tensor q, at::Tensor k,at::Tensor v, std::vector<std::vector<at::Tensor> >hidden_states,
  py::function attdense_noflash, py::function bias_dropout_add_func,py::function router_forward,py::function post_attention_layernorm,
  std::vector<at::Tensor> layernorm_input_xs, std::vector<std::vector<at::Tensor> > q_uses,
  std::vector<std::vector<at::Tensor> > layernorm_inputs, std::vector<std::vector<at::Tensor> >moe_inps
){

  for(int c = 0;c < chunk_size;c++){
    int bid = c / seq_groups;
    int sid = c % seq_groups;
    int batch_base = bid * real_batch;
    int base = sid * chunk_len;
    int slen = chunk_len;

    at::Tensor q_use = q.index({torch::indexing::Slice({base, base+slen}), torch::indexing::Slice({batch_base, batch_base+real_batch})});
    q_use.requires_grad_(true);

    at::Tensor hidden_ref = hidden_states[sid][bid];
    hidden_ref.requires_grad_(true);

    at::Tensor layernorm_input;

    {
      torch::AutoGradMode enable_grad(true);
      py::object att_out = attdense_noflash(q_use, k, v);
      std::vector<at::Tensor> att_out0 = att_out.cast<std::vector<at::Tensor> >();

      py::object layernorm_input_obj = bias_dropout_add_func(att_out0[0], att_out0[1].expand_as(hidden_ref),
      hidden_ref, hidden_dropout);

      at::Tensor  layernorm_input = layernorm_input_obj.cast<at::Tensor>();
    }
      at::Tensor layernorm_input_x = layernorm_input.detach().requires_grad_(true);
      layernorm_input_xs[c] = layernorm_input_x;

      q_uses[bid][sid] = q_use;
      layernorm_inputs[sid][bid] = layernorm_input;

    {
      torch::AutoGradMode enable_grad(true);
      py::object layernorm_output_obj = post_attention_layernorm(layernorm_input_x);
      at::Tensor layernorm_output = layernorm_output_obj.cast<at::Tensor>();

      at::Tensor moe_inp = layernorm_output.detach().requires_grad_(true);

      at::Tensor router_inp = layernorm_output.detach().requires_grad_(true);

      py::object outs = router_forward(router_inp);
...
at::Tensor cpp_one_hot_with_dtype(at::Tensor data, int num_classes, caffe2::TypeMeta dtype, int hot_value){
  at::Tensor result = torch::zeros({data.size(0), num_classes}, torch::dtype(dtype).device(data.device()));
...
std::vector<at::Tensor> set_Tensor(at::Tensor a){
...
std::tuple<int, std::vector<at::Tensor>, std::vector<at::Tensor>, std::vector<at::Tensor>, int, at::Tensor>
 cpp_extract_critical(at::Tensor scores, int top_k, int capacity_factor, int num_experts=-1){
  int num_global_experts = int(scores.size(1));
  int top_k_original = top_k;
  top_k = std::min(top_k, num_global_experts);

  at::Tensor topk_indices = std::get<1>(scores.topk(top_k, 1));
  std::vector<at::Tensor> chunks = topk_indices.chunk(top_k, 1);
  int counts = chunks.size();

  std::vector<at::Tensor> indices_s(counts);
  for(int i = 0;i < counts;i++){
    indices_s[i] = chunks[i].view(-1);
  }

  std::vector<at::Tensor> masks_se(counts);
  std::vector<at::Tensor> gates_s(counts);

  for(int i = 0;i < counts;i++){
    masks_se[i] = cpp_one_hot_with_dtype(indices_s[i], num_global_experts, indices_s[i].dtype(), 1);
    gates_s[i] = (scores * masks_se[i]).sum(1); 
  }

  at::Tensor locations1 = warp_cumsum(masks_se[0]);

  std::vector<at::Tensor> locations_s;
  locations_s.emplace_back(torch::sum(locations1 * masks_se[0], 1).to(torch::kInt32));

  if (top_k > 1){
    AT_ASSERTM(false, "topk must be one.");
  }

  for(int i = 0;i < counts;i++){
    indices_s[i] = indices_s[i].to(torch::kInt32);
  }

  int num_samples = int(scores.size(0));
  int samples_per_expert = (num_samples + num_global_experts - 1) / num_global_experts;

  int capacity;
  if(capacity_factor > 0){
    capacity = top_k * int(capacity_factor * samples_per_expert);
  }else{
    AT_ASSERTM(false, "capacity must be greater than 0.");
  }
  if (num_experts == -1) {
    return std::make_tuple(num_global_experts, indices_s, locations_s, gates_s, capacity, topk_indices);
  }
  at::Tensor tokens_per_expert = torch::histc(topk_indices.view(-1), num_experts);

  return std::make_tuple(num_global_experts, indices_s, locations_s, gates_s, capacity, tokens_per_expert);
}

std::vector<at::Tensor> pipe_backward(){
  
}

std::vector<at::Tensor> xpipe_forward(){
  
}

std::vector<at::Tensor> xpipe_backward(){
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("all_to_all_with_scale",
      &all_to_all_with_scale,
      "all_to_all_with_scale"
    );
...
#endif
  m.def("cpp_extract_critical", cpp_extract_critical);
  m.def("set_Tensor", set_Tensor);
  m.def("pipe_forward", pipe_forward);
  m.def("xpipe_forward", xpipe_forward);
  m.def("pipe_backward", pipe_backward);
  m.def("xpipe_backward", xpipe_backward);
}
...
extern "C" __global__ void cumsum_fn(int* input0 /* (num_samples, batch_num) */, int* output0 /* (num_samples, batch_num) */, int num_samples) {
  ...
  jit::jit_execute_with_values({x.data_ptr(), y.data_ptr(), (void*)x.size(0)}, cumsum_fn, x.device().index(), x.size(1), 1024, at::cuda::getCurrentCUDAStream().stream());
  return y;
}
