...
template<>
void run_mha_bwd_<cutlass::half_t, 64>(Flash_bwd_params &params, cudaStream_t stream, const int offset, const bool configure) {
    run_mha_bwd_hdim64<cutlass::half_t>(params, stream, offset, configure);
}