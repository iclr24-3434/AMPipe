...
inline __device__ void compute_dq_dk_dv_1colblock(const Params &params, const int bidb, const int bidh, const int n_block, const int offset_q) {
    ...
    // printf("n_block = %d, m_block_max = %d, seqq = %d, seqk = %d offset=%d-%d\n", n_block, m_block_max, binfo.actual_seqlen_q, binfo.actual_seqlen_k);
    //}
    ...
    int m_block_min = !Is_causal ? 0 : max((n_block * kBlockN) / kBlockM - offset_q, 0);
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_even_M, bool Is_even_K, typename Params>
inline __device__ void compute_dq_dk_dv_seqk_parallel(const Params &params, const int offset) {
    ...
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Is_even_M, Is_even_K, false, false, /*Seq_parallel=*/true>(params, bidb, bidh, n_block, offset);
}
