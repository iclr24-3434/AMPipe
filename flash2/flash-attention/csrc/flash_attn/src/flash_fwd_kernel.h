...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block, const int offset_q) {
    ...
    if (Is_causal) {
        n_block_max = std::min(n_block_max, cute::ceil_div((m_block + 1) * kBlockM + offset_q, kBlockN));
        //if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
        //     printf("m_block = %d, n_block_max = %d, seqq = %d, seqk = %d offset=%d-%d\n", m_block, n_block_max, binfo.actual_seqlen_q, binfo.actual_seqlen_k, (m_block + 1) * kBlockM, (m_block + 1) * kBlockM + offset_q);
         //}
    }
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_even_N, bool Is_even_K, bool Return_softmax, typename Params>
inline __device__ void compute_attn(const Params &params, const int offset) {
    ...
    flash::compute_attn_1rowblock<Kernel_traits, Is_dropout, Is_causal, Is_even_N, Is_even_K, Return_softmax>(params, bidb, bidh, m_block, offset);
}
