...
void run_mha_fwd(Flash_fwd_params &params, const int offset, cudaStream_t stream) {
    FP16_SWITCH(!params.is_bf16, [&] {
        FWD_HEADDIM_SWITCH(params.d, [&] {
            run_mha_fwd_<elem_type, kHeadDim>(params, offset, stream);
        });
    });
}
...
mha_varlen_fwd(const at::Tensor &q,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
    ...
    auto stream = at::cuda::getCurrentCUDAStream().stream();
    run_mha_fwd(params, offset, stream);
...
void run_mha_bwd(Flash_bwd_params &params, cudaStream_t stream, const int offset, const bool configure) {
    FP16_SWITCH(!params.is_bf16, [&] {
        if (params.d <= 32) {
            run_mha_bwd_<elem_type, 32>(params, stream, offset, configure);
        } else if (params.d <= 64) {
            run_mha_bwd_<elem_type, 64>(params, stream, offset,configure);
        } else if (params.d <= 96) {
            run_mha_bwd_<elem_type, 96>(params, stream, offset,configure);
        } else if (params.d <= 128) {
            run_mha_bwd_<elem_type, 128>(params, stream, offset,configure);
        } else if (params.d <= 160) {
            run_mha_bwd_<elem_type, 160>(params, stream, offset,configure);
        } else if (params.d <= 192) {
            run_mha_bwd_<elem_type, 192>(params, stream, offset,configure);
        } else if (params.d <= 224) {
          run_mha_bwd_<elem_type, 224>(params, stream, offset,configure);
        } else if (params.d <= 256) {
          run_mha_bwd_<elem_type, 256>(params, stream, offset,configure);
        }
    });
}
...
mha_bwd(const at::Tensor &dout,  // batch_size x seqlen_q x num_heads, x head_size_og
    ...
    launch(params, stream, 0, /*configure=*/false);
...
mha_varlen_bwd(const at::Tensor &dout,  // total_q x num_heads, x head_size
    ...
    launch(params, stream, offset, /*configure=*/false);
