...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    if iteration % args.log_interval == 0:
        a2a_1_time = timers('a2a-1').elapsed(reset=True)
        a2a_1_time_per_iteration = a2a_1_time / total_iterations

        a2a_2_time = timers('a2a-2').elapsed(reset=True)
        a2a_2_time_per_iteration = a2a_2_time / total_iterations

        mlp_time = timers('mlp').elapsed(reset=True)
        mlp_time_per_iteration = mlp_time / total_iterations

        attn_time = timers('attn').elapsed(reset=True)
        attn_time_per_iteration = attn_time / total_iterations

        block_time = timers('block').elapsed(reset=True)
        block_time_per_iteration = block_time / total_iterations

        block_bwd_time = timers('block-bwd').elapsed(reset=True)
        block_bwd_time_per_iteration = block_bwd_time / total_iterations
        ...
        log_string += ' a2a-1 time per iteration (ms): {:.1f} |'.format(
            a2a_1_time_per_iteration * 1000.0)
        log_string += ' a2a-2 time per iteration (ms): {:.1f} |'.format(
            a2a_2_time_per_iteration * 1000.0)
        log_string += ' mlp time per iteration (ms): {:.1f} |'.format(
            mlp_time_per_iteration * 1000.0)
        log_string += ' attn time per iteration (ms): {:.1f} |'.format(
            attn_time_per_iteration * 1000.0)
        log_string += ' block time per iteration (ms): {:.1f} |'.format(
            block_time_per_iteration * 1000.0)
        log_string += ' block-bwd time per iteration (ms): {:.1f} |'.format(
            block_bwd_time_per_iteration * 1000.0)
