...
class LearnedRouter(torch.nn.Module):
    ...
    def tutel_forwardx(self, x):
        if self.training and self.args.moe_jitter_eps is not None:
            x = x * self.jitter(x)

        sl, bs, hs = x.size()
        scores = self.layer(x.view(-1, hs)).softmax(dim=-1)
        return scores.detach().requires_grad_(), scores 
    ...
    def forwardx(self, x):
        ...
        sl, bs, hs = x.size()
        scores = self.layer(x.view(-1, hs)).softmax(dim=-1)
        scoresy = scores.detach().requires_grad_()
        scoresx = scores.detach().requires_grad_()
        if self.args.moe_top_k == 1:
            return scoresx, *scoresy.max(dim=-1), scoresy, scores
        return scoresx, *torch.topk(scoresy, self.args.moe_top_k, dim=-1), scoresy, scores
