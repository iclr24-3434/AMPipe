...
__global__ void fmha_bwd_dq_dk_dv_loop_kernel(FMHA_dgrad_params params, const int casual_offset_q) {
    fmha::compute_dq_dk_dv_1xN<Kernel_traits, Is_dropout, Is_causal, loop_steps>(params, casual_offset_q);
}

template<typename Kernel_traits, bool Is_dropout, bool Is_causal>
__global__ void fmha_bwd_q_dk_dv_loop_seqparallel_kernel(FMHA_dgrad_params params, const int casual_offset_q) {
    fmha::compute_dq_dk_dv_seqparallel<Kernel_traits, Is_dropout, Is_causal>(params, casual_offset_q);
}

template<typename Kernel_traits>
void run_fmha_bwd_loop(FMHA_dgrad_params &params, cudaStream_t stream, const bool configure, const int casual_offset_q) {
    ...
    BOOL_SWITCH(is_dropout, IsDropoutConst, ([&] {
        ...
        if (params.num_splits == 1) {
            ...
            kernel<<<grid, Kernel_traits::THREADS, smem_size_dq_dk_dv, stream>>>(params, casual_offset_q);
        } else {
            ...
            kernel_seqparallel<<<grid, Kernel_traits::THREADS, smem_size_dq_dk_dv, stream>>>(params, casual_offset_q);
        }
