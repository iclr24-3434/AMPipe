...
__global__ void fmha_fwd_loop_kernel(FMHA_fprop_params params, const int casual_offset_q) {
    fmha::device_1xN_loop<Kernel_traits, Is_dropout, Is_causal, Return_softmax>(params, casual_offset_q);
}

template<typename Kernel_traits>
void run_fmha_fwd_loop(Launch_params<FMHA_fprop_params> &launch_params, const int casual_offset_q) {
    ...
    BOOL_SWITCH(launch_params.is_dropout, IsDropoutConst, ([&] {
        ...
        kernel<<<grid, Kernel_traits::THREADS, smem_size, launch_params.stream>>>(
            launch_params.params, casual_offset_q);
