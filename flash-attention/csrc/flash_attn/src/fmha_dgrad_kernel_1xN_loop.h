...
inline __device__ void compute_dq_dk_dv_1xN_one_iter(const Params &params, Prng &ph,
                                                     const int loop_step_idx, const int casual_offset_q) {
    ...
    int begin = Is_causal ? max(0, (loop_step_idx * Cta_tile_p::N - casual_offset_q) / Cta_tile_p::M) : 0;
    ...
    for (int l = 0; l < steps; l++) {
        ...
        mask.load(begin + l, casual_offset_q);
        ...
        if (!Seq_parallel) {
            const bool is_final_write =
                ...
                || ((Is_causal) && ((begin + l) * Cta_tile_p::M + casual_offset_q < (loop_step_idx + 1) * Cta_tile_p::N));
            if (is_final_write) {
                // if (Is_dropout) {
...
inline __device__ void compute_dq_dk_dv_1xN(const Params &params, const int casual_offset_q) {
    ...
    if (loop_steps == 1) {
        compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, true, true>(params, ph, 0, casual_offset_q);
    } else if (loop_steps == 2) {
        compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, true, false>(params, ph, 0, casual_offset_q);
        compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, false, true>(params, ph, 1, casual_offset_q);
    } else {
        if (params.seqlen_k == blocksize_c) {
            ...
            for (int loop_step_idx = 1; loop_step_idx < max_loop_steps - 1; loop_step_idx++) {
                compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, false, false>(params, ph, loop_step_idx, casual_offset_q);
            }
            compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, false, true>(params, ph, max_loop_steps - 1, casual_offset_q);
        }
    }
}
...
inline __device__ void compute_dq_dk_dv_seqparallel(const Params &params, const int casual_offset_q) {
    // The block index for the batch.
    const int bidb = blockIdx.x;
    // The block index for the head.
    const int bidh = blockIdx.y;
    ...
    compute_dq_dk_dv_1xN_one_iter<Kernel_traits, Is_dropout, Is_causal, false, false, /*Seq_parallel=*/true>(params, ph, loop_step_idx, casual_offset_q);
}
