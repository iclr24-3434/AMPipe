...
torch::Tensor fake_tensor(torch::Tensor in){
  auto options = torch::TensorOptions().device(in.device()).dtype(in.dtype());
  auto out = torch::from_blob(in.data_ptr(),in.sizes(), options);
  return out;
}
